...
# Amir Samary - 2018
#
# All this script does is to run the Installer Class specified on
# environment variable IRIS_INSTALLER_CLASSNAME. The class will be run on
# %SYS namespace as it should be. The user name and password used to load
...
printf "%s\n%s\nzn \"%s\"\nSet tSC=\$system.OBJ.Load(\"%s\",\"ck\")\n$VerifySC\n" "$IRIS_USERNAME" "$IRIS_PASSWORD" "%SYS" "/tmp/appinstaller.cls" | irissession IRIS

printf "\n\nRunning /tmp/appinstaller.cls..."
printf "%s\n%s\n%s\n%s\n" "$IRIS_USERNAME" "$IRIS_PASSWORD" "zn \"%SYS\"" "Do ##class($IRIS_INSTALLER_CLASSNAME).Install()" | irissession IRIS
