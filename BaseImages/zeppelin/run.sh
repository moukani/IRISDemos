...
#
function showHelp() {
    printfY "\n\nStart Zeppelin container locally, configured to connect to an IRIS server"
    printfY "\nthrough JDBC and/or Spark. A folder called ./shared will be "
    printfY "\ncreated to hold the custom configurations and zeppelin notebook files."
    printf "\n\nParameters: "
    printfY "\n"
    printfY "\n\t-h | --help"
    printf "\n\tShows this help."
...
printfY "\n\nRunning container... In a couple of seconds, Zeppelin should be available on http://localhost:9090/\n"
docker run $EPHEMERALFLAG -it  \
    -p 9090:9090 -p 9091:8080 -p 9092:8081 \
    -v $PWD/shared:/shared \
